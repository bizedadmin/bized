...
export function BookingEditor() {
    ...
    useEffect(() => {
        if (currentBusiness?.socialLinks?.whatsappTemplates?.booking) {
            setTemplate(currentBusiness.socialLinks.whatsappTemplates.booking);
        }
    }, [currentBusiness]);
    ...
    const renderPreviewTemplate = (text: string) => {
        if (!text) return "Hi! I'd like to book {{product_name}}.";
        return text
            .replace(/\{\{business_name\}\}/g, currentBusiness?.name || "My Business")
            .replace(/\{\{product_name\}\}/g, "Premium Consultation")
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="bg-[var(--color-surface)] border border-[var(--color-outline-variant)]/20 rounded-[2.5rem] p-8 shadow-sm space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold">Booking Journey</h3>
                        <p className="text-sm text-[var(--color-on-surface-variant)] opacity-60">Customize the message customers send when booking.</p>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        <div className="flex items-center justify-between px-1">
                            <label className="text-xs font-bold text-[var(--color-on-surface-variant)] opacity-50 uppercase tracking-widest">WhatsApp Booking Template</label>
                        ...
                        <div className="relative group">
                            ...
                            <textarea
                                ...
                                placeholder="Hi! I'd like to book {{product_name}}..."
            ...
            <div className="bg-[var(--color-surface-container-highest)]/30 rounded-[2.5rem] border border-[var(--color-outline-variant)]/10 p-8 space-y-6">
                ...
                <div className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-4 flex gap-3">
                    ...
                    <p className="text-[11px] text-amber-900/60 leading-relaxed">
                        <strong>Pro Tip:</strong> Products set to "Booking" type will automatically show a "Book" button and use this template for customer inquiries.
                    </p>
                </div>
            </div>
        </div>
